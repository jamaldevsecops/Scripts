...
get_system_info() {
  ...
  ssh -o ConnectTimeout=5 -p "$ssh_port" "$ssh_user@$host" "
    hostname
    lsb_release -d | grep -i 'description' | cut -d: -f2 | xargs || grep -i 'PRETTY_NAME' /etc/*release | cut -d= -f2 | xargs
    uname -r
    grep -c ^processor /proc/cpuinfo
    free -m | awk '/Mem:/ {print \$2}'
    lsblk -d -n -b -o SIZE | awk '{total += \$1} END {print total/(1024*1024*1024)}'
  " 2>/dev/null
}
...
for host in $hosts; do
  ...
  if [ -n "$system_info" ]; then
    ...
    os_type=$(echo "$system_info" | sed -n '2p')
