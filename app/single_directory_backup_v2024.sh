...
# Create backup for the specific directory and update the report
backup_filename="$(basename "$backup_dir")_${today}.tar.gz"
tar -czf "${daily_backup_dir}/$backup_filename" -C "$(dirname "$backup_dir")" "$(basename "$backup_dir")"
if [ $? -eq 0 ]; then
    message+="\n${backup_dir}: Success"
...
file_to_delete=$(basename "$backup_dir")_${dr}.tar.gz
...
file_to_delete=$(basename "$backup_dir")_${wr}.tar.gz
...
file_to_delete=$(basename "$backup_dir")_${mr}.tar.gz
...
file_to_delete=$(basename "$backup_dir")_${yr}.tar.gz
