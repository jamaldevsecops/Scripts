...
backup_filename="${SRC_DB}_${today}.sql.gz"

mysqldump --skip-lock-tables --skip-add-locks -h "$SRC_HOST" -P "$SRC_PORT" -u "$SRC_USER" --password="$SRC_PASS" -R "$SRC_DB" | gzip > "${daily_backup_dir}/${backup_filename}" 2>/dev/null
if [ $? -eq 0 ]; then
    message+="\n${SRC_DB}: Success"
